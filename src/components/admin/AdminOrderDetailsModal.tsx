...
export const AdminOrderDetailsModal: React.FC<Props> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-xs animate-in fade-in overflow-y-auto">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-3xl shadow-2xl overflow-hidden my-6 animate-in zoom-in-95">
        {/* Modal Top Bar */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 space-y-3">
              <div className="flex items-center justify-between pb-2 border-b border-slate-800">
                ...
                <span className="text-[10px] bg-slate-800 text-slate-200 font-bold px-2 py-0.5 rounded">
                  COD Verified
                </span>
              </div>
              ...
              <div className="space-y-1.5 text-xs">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-mono font-bold text-slate-100">{order.phone}</span>
                </div>
              ...
              <div className="flex items-center gap-2 pt-2 border-t border-slate-800">
                <a
                  ...
                  className="flex-1 py-2 px-3 bg-slate-800 hover:bg-slate-700 text-slate-100 font-bold text-xs rounded-xl flex items-center justify-center gap-1.5 transition-colors"
                >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="flex-1 py-2 px-3 bg-emerald-950 hover:bg-emerald-900 border border-emerald-800 text-emerald-300 font-bold text-xs rounded-xl flex items-center justify-center gap-1.5 transition-colors"
                >
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 space-y-3">
              <div className="flex items-center justify-between pb-2 border-b border-slate-800">
                <span className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                  <Truck className="w-3.5 h-3.5 text-indigo-400" /> Courier & Payment
                </span>
              ...
              <div className="space-y-2 text-xs">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-slate-400">Courier Delivery Fee:</span>
                  <span className="font-mono font-bold text-white">৳{order.deliveryFee}</span>
                </div>
                <div className="p-2.5 bg-amber-950/70 border border-amber-800/80 rounded-xl">
                  <span className="text-[11px] font-bold text-amber-300 block mb-0.5">Special Courier Notes:</span>
          ...
          <div className="space-y-3">
            ...
            <div className="border border-slate-800 rounded-2xl overflow-hidden divide-y divide-slate-800">
              {order.items.map((item) => (
                <div key={item.id} className="p-3.5 flex items-center gap-4 hover:bg-slate-950 transition-colors">
                  <div className="w-16 h-16 bg-black rounded-xl overflow-hidden shrink-0 border border-slate-800">
                    <TShirtMockupView
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="text-xs font-black text-white truncate">{item.product.title}</h4>
                    <div className="flex items-center gap-3 text-xs text-slate-400 mt-1">
                      <span className="font-bold text-slate-200">
                        Size: <b className="text-indigo-400 bg-indigo-950 px-1.5 py-0.5 rounded">{item.selectedSize}</b>
                      </span>
